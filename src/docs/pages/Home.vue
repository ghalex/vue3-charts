<template>
  <HomeLayout>
    <div class="flex justify-center">
      <Chart class="mx-2" :width="900" :height="350" :data="data" :margin="margin">
        <Grid strokeDasharray="3,3" />
        <YMarker :value="0" color="#000" strokeDasharray="0" strokeWidth="1" strokeOpacity="1" />
        <Bar dataKey="pl" :fill="barFill" :maxWidth="20" />
        <Bar dataKey="avg" fill="red" :maxWidth="30" />
        <XAxis />
        <YAxis :ticks="8" />
        <template #widgets>
...
export default defineComponent({
  ...
  setup() {
    ...
    const barFill = (props: any) => {
      return props.y > 0 ? '#82ca9d' : '#b82c0d'
    }
